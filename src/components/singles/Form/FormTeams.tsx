...
// Components (local)
import LinkBrand from '@/components/singles/Link/LinkBrand';

/*---------- Static Data ----------*/

// Name
const name = 'FormTeams';

/*---------- Template ----------*/

// Types
export type FormTeamsProps = {
	className?: string;
};

// Default component
export default function FormTeams(props: FormTeamsProps) {
	/*----- Props -----*/
		...
		<div className={classNames(`form space-y-6`, className)} data-name={name}>
			<div className="form__row space-y-6">
				<div className="form__title flex flex-col justify-between space-y-8 sm:flex-row sm:space-y-0 sm:space-x-6 sm:text-left">
					<h2 className="text-4xl font-bold tracking-tight">Teams</h2>
					{auth.role === 'moderator' && (
						...
						>
							<div className="form__item-col flex min-w-0 gap-x-4 text-left">
								...
								<div className="min-w-0 flex flex-col justify-center">
									...
									{team.brands && team.brands.length > 0 && (
										<p className="form__brands mt-1 truncate text-xs leading-5 text-gray-500">
											{team.brands.map((id, b) => (
												<>
													<LinkBrand key={`form-teams-brand-${id}`} id={id} />
													<span
														key={`form-teams-comma-${id}`}
